...
*/
class GoogleSearchFilteredBridge extends BridgeAbstract {

	const MAINTAINER = "sebsauvage";
	const NAME = "Google search filtered";
	...
	const STOPWORDS = "(www.rebelle.com|www.fondazioneprada.org)";
	...
	public function collectData() {
		...
		if( !is_null($emIsRes) ) {
			...
			foreach($emIsRes->find('div[class=g]') as $element) {
                ...
                if(preg_match(static::STOPWORDS, $item['uri']) === 1) {
	...
	public function getName(){
		...
		return $this->getInput('q') .' - Google search filtered';
	}
}
